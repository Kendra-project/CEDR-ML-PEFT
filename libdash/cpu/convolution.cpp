...
void DASH_CONV_2D_flt_cpu(dash_re_flt_type **input, int *height, int *width, dash_re_flt_type **mask, int *mask_size, dash_re_flt_type **output) {
  int i, j, k, l;
  int s, w;
  int z;
...
void DASH_CONV_2D_int_cpu(dash_re_int16_type **input, int *height, int *width, dash_re_flt_type **mask, int *mask_size, dash_re_int16_type **output) {
  int i, j, k, l;
  int s, w;
  int z;
  ...
  for (i = 0; i < (*height); i++) {
    for (j = 0; j < (*width); j++) {
      ...
      (*output)[i * (*width) + j] = (int16_t)( (sum > CONV_2D_MAX) ? CONV_2D_MAX : ( (sum < CONV_2D_MIN) ? CONV_2D_MIN : sum));
    }
  }
}
...
void DASH_CONV_2D_flt_nb(dash_re_flt_type **input, int *height, int *width, dash_re_flt_type **mask, int *mask_size, dash_re_flt_type **output, cedr_barrier_t* kernel_barrier) {
  ...
  DASH_CONV_2D_flt_cpu(input, height, width, mask, mask_size, output);
  if (kernel_barrier != nullptr) {
    (*(kernel_barrier->completion_ctr))++;
  }
#else
  enqueue_kernel("DASH_CONV_2D", "flt", 7, input, height, width, mask, mask_size, output, kernel_barrier);
#endif
}
...
void DASH_CONV_2D_flt(dash_re_flt_type *input, int height, int width, dash_re_flt_type *mask, int mask_size, dash_re_flt_type *output) {
  ...
  DASH_CONV_2D_flt_nb(&input, &height, &width, &mask, &mask_size, &output, &barrier);

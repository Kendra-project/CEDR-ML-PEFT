...
__constant__ dash_re_flt_type mask[1024];


extern "C" __global__ void conv2D_flt(const dash_re_flt_type* input, dash_re_flt_type* output, int width, int height, int mask_size, int tile_size) {
  dash_re_flt_type result = 0;
  ...
  __shared__ dash_re_flt_type TILE[BLOCK_SIZE][BLOCK_SIZE];
  ...
  if(threadIdx.y < tile_size && threadIdx.x < tile_size){
    ...
    if(ROW_out < height && COL_out < width){
      output[ROW_out * width + COL_out] = ( (result > CONV_2D_MAX) ? CONV_2D_MAX : ( (result < CONV_2D_MIN) ? CONV_2D_MAX : result));//floorf(result);
    }
  }
}

extern "C" void DASH_CONV_2D_flt_gpu(dash_re_flt_type** input, int *height_in, int *width_in, dash_re_flt_type **mask_in, int *mask_size_in, dash_re_flt_type **output) {
//    printf("---------------------------------------\n");
...
//    printf("---------------------------------------\n");
  ...
  const int size_in_bytes = width * height * sizeof(dash_re_flt_type);
  const int size_in_bytes_mask = mask_size * mask_size * sizeof(dash_re_flt_type);
  cudaError_t err = cudaSuccess;

  dash_re_flt_type* d_input;
  dash_re_flt_type* d_output;
/*  dash_re_flt_type* d_mask = NULL;*/
  err = cudaMalloc((void**)&d_input, size_in_bytes);
//  err = cudaGetLastError();

  if (err != cudaSuccess) {
    fprintf(stderr, "Failed cudaMalloc for conv2D on gpu (error code %s)%d\n", cudaGetErrorString(err),size_in_bytes);
  }
  err = cudaMalloc((void**)&d_output, size_in_bytes);
//  err = cudaGetLastError();

  if (err != cudaSuccess) {
    fprintf(stderr, "Failed cudaMalloc for conv2D on gpu (error code %s)\n", cudaGetErrorString(err));
  }
...
//  err = cudaGetLastError();
  ...
  if (err != cudaSuccess) {
    fprintf(stderr, "Failed cudaMemcpy for conv2D on gpu (error code %s)\n", cudaGetErrorString(err));
  }
...
//  err = cudaGetLastError();
  ...
  if (err != cudaSuccess) {
    fprintf(stderr, "Failed cudaMemcpyToSymbol for conv2D on gpu (error code %s)\n", cudaGetErrorString(err));
  }

  conv2D_flt<<<grid_dim, block_dim>>>(d_input, d_output, width, height, mask_size, tile_size);

  err = cudaGetLastError();

  if (err != cudaSuccess) {
    fprintf(stderr, "Failed to launch kernel for conv2D on gpu (error code %s)\n", cudaGetErrorString(err));
  }
...
extern "C" __global__ void conv2D_int(const int16_t* input, int16_t* output, int width, int height, int mask_size, int tile_size) {
  double result = 0;
  int COL_out = threadIdx.x + blockIdx.x * tile_size; // Col of the thread
  int ROW_out = threadIdx.y + blockIdx.y * tile_size; // Row of the thread
...
extern "C" void DASH_CONV_2D_int_gpu(int16_t** input, int *height_in, int *width_in, float **mask_in, int *mask_size_in, int16_t **output) {
//    printf("---------------------------------------\n");
//    printf("----------- CONV-2D on GPU ------------\n");
//    printf("---------------------------------------\n");
  const int width = *width_in;
...
//  err = cudaMemcpy(d_mask, (*mask_in), size_in_bytes_mask, cudaMemcpyHostToDevice);
  ...
  conv2D_int<<<grid_dim, block_dim>>>(d_input, d_output, width, height, mask_size, tile_size);

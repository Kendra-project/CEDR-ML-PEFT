...
private:
  ...
  bool log_traces;
  bool periodic_logs;
  bool overwrite_periodic_log;
  unsigned int periodic_logging_threshold;
  uint64_t  logging_period;
  unsigned int total_num_of_resources;
  uint64_t app_logging_threshold;
  bool use_perf;
...
public:
  ...
  bool getLogTraces() const;
  bool getPeriodicLogs() const;
  bool getOverwritePeriodicLog() const;
  unsigned int getPeriodicLoggingThreshold() const;
  uint64_t getLoggingPeriod() const;
  uint64_t getAppLoggingThreshold() const;
  bool getUsePERF() const; 

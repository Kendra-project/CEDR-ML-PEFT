...
void *hardware_thread(void *ptr) {
  ...
  if (cedr_config->getUsePERF()){
    LOG_INFO << "Initializing PERF on thread " << self;
  ...
  CUcontext cuCtx;
  CUdevice cuDev;
  const int numEvents = sizeof(eventNames) / sizeof(eventNames[0]);
  CUpti_EventGroup eventGroup;
  CUpti_EventID eventIds[numEvents];
  uint32_t instanceCount;
  size_t sz = sizeof(instanceCount);
  std::vector<uint64_t> startValues(numEvents);
  std::vector<uint64_t> endValues(numEvents);
  if (worker_thread->thread_resource_type == resource_type::gpu){
    cudaFree(0);
    cuCtxGetCurrent(&cuCtx);
    cuCtxGetDevice(&cuDev);
    cuptiEventGroupCreate(cuCtx, &eventGroup, 0);
    for (int i = 0; i < numEvents; ++i) {
      cuptiEventGetIdFromName(cuDev, eventNames[i], &eventIds[i]);
      cuptiEventGroupAddEvent(eventGroup, eventIds[i]);
    }
    cuptiEventGroupEnable(eventGroup);
    cuptiEventGroupGetAttribute(eventGroup, CUPTI_EVENT_GROUP_ATTR_INSTANCE_COUNT, &sz, &instanceCount);
  }
#endif
  ...
  *is_sleeping = true;
  ...
  while (true) {
    ...
    if ((!worker_thread->todo_task_dequeue.empty()) && (worker_thread->resource_state != 3)) {
      ...
      #if defined(ANOMALY_DETECTION)
      if(worker_thread->task->app_pnt->anomaly_or_not == true){
        pthread_mutex_unlock(thread_lock);
        //cedr_barrier_t *barrier = task->kernel_barrier;
        //pthread_mutex_lock(barrier->mutex);
        //*(barrier->completion_ctr) = *(barrier->completion);
      ...
      if (args.size() >= MAX_ARGS) {
        ...
        if (worker_thread->thread_resource_type == resource_type::gpu){
          for (int i = 0; i < numEvents; ++i) {
            std::vector<uint64_t> tmp(instanceCount);
            size_t valSize = instanceCount * sizeof(uint64_t);
            cuptiEventGroupReadEvent(eventGroup, CUPTI_EVENT_READ_FLAG_NONE, eventIds[i], &valSize, tmp.data());
            uint64_t total = 0;
            for (uint32_t k = 0; k < instanceCount; ++k){ total += tmp[k]; }
            startValues[i] = total;
          }
          /*for (int i = 0; i < numEvents; ++i) {
            printf("Start Value  %s: %llu\n", eventNames[i], (unsigned long long) startValues[i]);
          }*/
        }
#endif
#if defined(USEPAPI)
        ...
        if (cedr_config->getUsePERF()){
          for (int i =0; i < PERF_EVENT_NUM; i++){
            ioctl(FILE_DESCRIPTOR[i], PERF_EVENT_IOC_RESET, 0);
            ioctl(FILE_DESCRIPTOR[i], PERF_EVENT_IOC_ENABLE, 0);}
        }
        LOG_INFO << "Reseting perf counters on worker thread " << self;
#endif
...
#endif
#if defined(ANOMALY_DETECTION)
        uint64_t cur_exec = (task->end - task->start);
        worker_thread->total_runtime += cur_exec;
        // Moved `pthread_cond_signal` below task exec calculations!
        pthread_mutex_lock(barrier->mutex);
        task->app_pnt->min_task = task->app_pnt->min_task > cur_exec ? cur_exec : task->app_pnt->min_task;
        task->app_pnt->max_task = task->app_pnt->max_task > cur_exec ? task->app_pnt->max_task : cur_exec;
        task->app_pnt->total_task += cur_exec;
        pthread_mutex_unlock(barrier->mutex);
 #if defined(USEPERF)
        if (cedr_config->getUsePERF()){
          unsigned long long perf_counter[PERF_EVENT_NUM];
          for (int i =0; i < PERF_EVENT_NUM; i++){
            ioctl(FILE_DESCRIPTOR[i], PERF_EVENT_IOC_DISABLE, 0);
          }
          for (int i =0; i < PERF_EVENT_NUM; i++){
            read(FILE_DESCRIPTOR[i],&(perf_counter[i]), sizeof(uint64_t));
            worker_thread->task->app_pnt->API_perf_counters[i] += perf_counter[i];
          }
          LOG_INFO << "On worker thread " << self << "there were " << perf_counter[0] << " " << perf_events_string[0] << " and " << perf_counter[1] << " " << perf_events_string[1];
        }
#endif       
#endif
        pthread_mutex_lock(barrier->mutex);
      ...
      if (worker_thread->thread_resource_type == resource_type::gpu){
        ...
        /*for (int i = 0; i < numEvents; ++i) {
          printf("End Value  %s: %llu\n", eventNames[i], (unsigned long long) endValues[i]);
        }*/
        //printf("[App %d] Metrics:\n", 0);
        for (int i = 0; i < numEvents; ++i) {
          worker_thread->task->app_pnt->APP_cupti_counters[i] += (unsigned long long)(endValues[i] - startValues[i]);
          worker_thread->task->API_cupti_counters[i] = (unsigned long long)(endValues[i] - startValues[i]);
          //printf("  %s: %llu\n", eventNames[i], (unsigned long long)(endValues[i] - startValues[i]));
        }
      }
#endif

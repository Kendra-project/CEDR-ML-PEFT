...
void nk_thread_func(void* app_pnt) {
  cedr_app* app = (cedr_app*) app_pnt;
#if defined(USEPERF)
  struct perf_event_attr PERF_EA[PERF_EVENT_NUM]; // Might not be needed, can just use single variable
  int FILE_DESCRIPTOR[PERF_EVENT_NUM];
  LOG_INFO << "Initializing PERF on " << app->app_name << " with app ID " << app->app_id;
  for (int i=0; i < PERF_EVENT_NUM; i++){
    memset(&(PERF_EA[i]), 0, sizeof(struct perf_event_attr));
    PERF_EA[i].type = PERF_TYPES[perf_event_types[i]];
    PERF_EA[i].size = sizeof(struct perf_event_attr);
    PERF_EA[i].config = perf_events[i];
    PERF_EA[i].disabled = 1;
    PERF_EA[i].exclude_kernel = 1;
    PERF_EA[i].exclude_hv = 1;
    FILE_DESCRIPTOR[i] = perf_event_open(&(PERF_EA[i]), 0, -1, -1, 0);
    if(FILE_DESCRIPTOR[i] == -1){
      LOG_ERROR << "PERF error " << FILE_DESCRIPTOR[i] << " " << strerror(errno) << " on " << perf_events_string[i];
    }
    app->perf_fd[i] = FILE_DESCRIPTOR[i];
    ioctl(app->perf_fd[i], PERF_EVENT_IOC_RESET, 0);
    ioctl(app->perf_fd[i], PERF_EVENT_IOC_ENABLE, 0);
  }
  LOG_INFO << "Reseting perf counters on " << app->app_name << " with app ID " << app->app_id;
#endif
  ...
  int (*libmain)(int, char**, char**) = (int(*)(int, char**, char**)) app->main_func_handle;  // Call the library's main
...
void writeTimingTrace(FILE* trace_fp, ConfigManager &cedr_config, worker_thread *hardware_thread_handle, bool skipPEs, uint64_t* earliestStart, uint64_t* latestFinish){
  ...
  for (unsigned int i = 0; i < total_resource_count; i++) {
    ...
    if(skipPEs){
      if (completed_task_queue_length <= cedr_config.getPeriodicLoggingThreshold()){
        // Wait until PEs completed task queue reaches to the log counter
        continue;
      }
    }
    if (trace_fp == nullptr) {
      ...
      while (completed_task_queue_length != 0) {
        ...
        fprintf(trace_fp,
          ...
          log_obj.app_id, log_obj.app_name, log_obj.task_id, log_obj.task_name, log_obj.assign_resource_name, s0, e0, e0 - s0);

        free(task->task_name.data());
        free(task->assigned_resource_name.data());
        free(task->args.data());
        free(task);
...
void writeApplicationTrace(FILE* apptrace_fp, ConfigManager &cedr_config,  worker_thread *hardware_thread_handle, std::list<cedr_app *> *completed_nonstreaming_apps, uint64_t* earliestStart, uint64_t* latestFinish, FILE* trace_fp
#if defined(ANOMALY_DETECTION)
	,uint64_t* total_makespan_start
	,uint64_t* total_makespan_end
	,std::list<struct struct_adfeaturelogging> *ad_feature_log
#endif
		){
  ...
  if (apptrace_fp == nullptr){
    ...
    // Write task logs before freeing the application pointers for recording the task ID and name properly
    writeTimingTrace(trace_fp, cedr_config, hardware_thread_handle, false, earliestStart, latestFinish);
    int logging_threshold = cedr_config.getPeriodicLoggingThreshold();
...
#if defined(ANOMALY_DETECTION)
      *total_makespan_start = applog_obj.start > *total_makespan_start ? *total_makespan_start : applog_obj.start;
      *total_makespan_end = applog_obj.end > *total_makespan_end ? applog_obj.end : *total_makespan_end;
      struct struct_adfeaturelogging featurelog_obj {};
      strncpy(featurelog_obj.app_name, applog_obj.app_name, 50);
      featurelog_obj.app_id = applog_obj.app_id;
      featurelog_obj.arrival = applog_obj.arrival;
      featurelog_obj.start = applog_obj.start;
      featurelog_obj.end = applog_obj.end;
      featurelog_obj.app_runtime = applog_obj.app_runtime;
      featurelog_obj.app_lifetime = applog_obj.app_lifetime;
      featurelog_obj.task_count = erase_app->task_count;
      featurelog_obj.max_in_ready_queue = erase_app->max_in_ready_queue;
      featurelog_obj.max_in_todo_queue = erase_app->max_in_todo_queue;
      featurelog_obj.PE_todo_queue = (unsigned int *)calloc(cedr_config.getTotalResources(), sizeof(unsigned int));
      for (int i = 0; i < cedr_config.getTotalResources(); i++){
        featurelog_obj.PE_todo_queue[i] = erase_app->PE_todo_queue[i];
      }
      featurelog_obj.file_size = erase_app->file_size;
      featurelog_obj.min_task = erase_app->total_task == 0 ? 0 : erase_app->min_task;  //erase_app->min_task; 
      featurelog_obj.max_task =  erase_app->total_task == 0 ? 0 : erase_app->max_task;  //erase_app->max_task;
      featurelog_obj.mean_task = erase_app->total_task == 0 ? 0.0f : (float) erase_app->total_task / (float) erase_app->task_count; //(float) erase_app->total_task / (float) erase_app->task_count;
      #if defined(USEPERF)
      if (cedr_config.getUsePERF()){
        unsigned long long perf_counter[PERF_EVENT_NUM];
        for (int i=0; i < PERF_EVENT_NUM; i++){
          ioctl(erase_app->perf_fd[i], PERF_EVENT_IOC_DISABLE, 0);
        }
        for (int i=0; i < PERF_EVENT_NUM; i++){
          read(erase_app->perf_fd[i], &(perf_counter[i]), sizeof(uint64_t));
          erase_app->APP_perf_counters[i] = perf_counter[i];
          close(erase_app->perf_fd[i]);
        }
      }
      #endif      
      #if defined(USEPERF)
      if (cedr_config.getUsePERF()){
        for (int i=0; i < PERF_EVENT_NUM; i++){
          featurelog_obj.API_perf_counters[i] = erase_app->API_perf_counters[i];
          featurelog_obj.APP_perf_counters[i] = erase_app->APP_perf_counters[i];
        }
      }
      #endif     
      #if defined(CUPTI_LOGGING)
        for (int i=0; i < CUPTI_EVENT_NUM; i++){
          featurelog_obj.APP_cupti_counters[i] = erase_app->APP_cupti_counters[i];
        }
      #endif
      ad_feature_log->push_back(featurelog_obj);
#endif
      ...
      fprintf(apptrace_fp, "app_id: %d, app_name: %s, ref_arrival_time: %" PRIu64 ", ref_start_time: %" PRIu64 ", ref_end_time: %" PRIu64 ", app_runtime: %" PRIu64 ", app_lifetime: %" PRIu64 " \n",
              applog_obj.app_id, applog_obj.app_name, applog_obj.arrival, applog_obj.start, applog_obj.end, applog_obj.app_runtime, applog_obj.app_lifetime);

      completed_nonstreaming_apps->remove(erase_app);
      free(erase_app);
      // Exit if logging threshold is reached
      logging_counter++;
      if(logging_counter == logging_threshold){
	      break;
      }
    }
  }
...
void launchDaemonRuntime(ConfigManager &cedr_config, pthread_t *resource_handle, worker_thread *hardware_thread_handle) {
 ...
 // float input_data[42*n_samples];
//  float output_data[42*n_samples];
 // std::fill_n(input_data, 42*n_samples, 10.0f);
 // std::fill_n(output_data, 42*n_samples, 50.0f);
 // const char* command = "./zcu102_power_test.elf -d -n 50 -t 1";
//  struct timespec start_ae, end_ae;
//  unsigned long long total_latency = 0; // Total latency over all iterations
  // Repeat the operation for 1000 times
  //std::cout << "power results before the model is running" << std::endl;
  //std::future<int> result = std::async(std::launch::async, std::system, command);
    
  //result.get(); // Wait for the asynchronous task to complete  // Wait for the user to press Enter to continue
  //std::cout << "power results while the model is running" << std::endl;
  //result = std::async(std::launch::async, std::system, command);
  //for (int i = 0; i < 1000; i++)
  //autoencoder_kernel(input_data, output_data, n_samples);
  //result.get();
  //std::cout << "power analysis is done" << std::endl;
  
  //exit(0); // or return 0; if it's within the main function

  /*for (int i = 0; i < 10; ++i) {
      clock_gettime(CLOCK_MONOTONIC_RAW, &start_ae);
      autoencoder_kernel(input_data, output_data, n_samples);
      clock_gettime(CLOCK_MONOTONIC_RAW, &end_ae);

      unsigned long long latency_ns = 
          (end_ae.tv_sec - start_ae.tv_sec) * 1000000000 +
          (end_ae.tv_nsec - start_ae.tv_nsec);
      
      total_latency += latency_ns;
  }  
  
  float average_latency_us = static_cast<float>(total_latency / n_samples) / 10000.0f;

  std::cout << "Average Hardware Autoencoder latency per sample: " << average_latency_us << " microseconds" << std::endl;
  ...
  */
...
#if defined(ANOMALY_DETECTION)
  std::list<struct struct_adfeaturelogging> ad_feature_log;
  std::list<struct struct_adfeaturelogging> ad_feature_log_periodic;
  uint64_t total_makespan_start=0xFFFFFFFFFFFFFFFF;
  uint64_t total_makespan_end=0;
#endif
  unsigned int current_ready_queue_size = 0;
  uint64_t clock_vir0, clock_vir1;
  ...
  }
  {
    ...
    if(cedr_config.getLogTraces() && cedr_config.getPeriodicLogs()){
      #if defined(TASK_LOGGING)
      trace_fp = fopen((log_path + "/timing_trace.log").c_str(), "a");
      schedtrace_fp = fopen((log_path + "/schedule_trace.log").c_str(), "a");
      #endif 
      apptrace_fp = fopen((log_path + "/appruntime_trace.log").c_str(), "a");
    }
#if defined(ANOMALY_DETECTION)
    //if(cedr_config.getPeriodicLogs()){
    //  ad_features_fp = fopen((log_path + "/ad_features.csv").c_str(), "a");
    //  if(ad_features_fp == nullptr){
    //    LOG_ERROR << "Error outputting anomaly detection feature file!";
    //  }
    //  fprintf(ad_features_fp, "app_id,app_name,app_size,app_runtime,app_task_count,max_in_ready_queue,max_in_todo_queue,ref_arrival_time,app_diff_arrival_time,app_lifetime,app_api_min,app_api_max,app_api_mean,app_is_running");
    //  for (int i = 0; i < cedr_config.getTotalResources(); i++) {
    //   fprintf(ad_features_fp, ",%s_todo_queue", hardware_thread_handle[i].resource_name.c_str());
    //  }
    //  for (int i = 0; i < cedr_config.getTotalResources(); i++) {
    //   fprintf(ad_features_fp, ",%s", hardware_thread_handle[i].resource_name.c_str());
    //  }
    //  fprintf(ad_features_fp, "\n");
    //}
#endif
  ...
  prev_emul_time = cedrGetTime(hardware_thread_handle[0].time_per_cycle);
  ...
  while (true) {
    ...
    if (ipc_cmd.cmd_type == SH_OBJ) { // Command for new application received
      ...
      while (prototypeAppPtr && (new_apps_instantiated < num_inst)) {
        ...
        *new_app = *prototypeAppPtr;
        ...
        #if defined(ANOMALY_DETECTION)
          new_app->file_size = st.st_size;
          new_app->max_in_ready_queue = 0;
          new_app->max_in_todo_queue = 0;
          new_app->PE_todo_queue = (unsigned int *)calloc(cedr_config.getTotalResources(), sizeof(unsigned int));
          for (int i = 0; i < cedr_config.getTotalResources(); i++){// Newly added in order not to get incorrect and large todo_queue values.
            new_app->PE_todo_queue[i] = 0;
          }
          new_app->min_task = 0xFFFFFFFFFFFFFFFF;
          new_app->max_task = 0;
          new_app->total_task = 0;
          new_app->anomaly_or_not = false;
          #if defined(USEPERF)
          if (cedr_config.getUsePERF()){
            for (int i=0; i < PERF_EVENT_NUM; i++){
              new_app->API_perf_counters[i] = 0;
              new_app->APP_perf_counters[i] = 0;
            }
          }
          #endif          
          #if defined(CUPTI_LOGGING)
            for (int i=0; i < CUPTI_EVENT_NUM; i++){
              new_app->APP_cupti_counters[i] = 0;
            }
          #endif
          #endif
      ...
      for (auto task : poison_queue){
        ...
        printf("Entering poison_queue for loop!!!\n");
        #if defined(USEPERF)
        if (cedr_config.getUsePERF()){
          unsigned long long perf_counter[PERF_EVENT_NUM];
          for (int i=0; i < PERF_EVENT_NUM; i++){
            ioctl(task->app_pnt->perf_fd[i], PERF_EVENT_IOC_DISABLE, 0);
          }
          for (int i=0; i < PERF_EVENT_NUM; i++){
            read(task->app_pnt->perf_fd[i], &(perf_counter[i]), sizeof(uint64_t));
            task->app_pnt->APP_perf_counters[i] = perf_counter[i];
            close(task->app_pnt->perf_fd[i]);
          }
        }
        #endif
        resolved_app_count++;
	...
	if(cedr_config.getLogTraces()){
      ...
      if (arrived_nonstreaming_apps.size() != 0){
        ...
        while (arrived_nonstreaming_apps.size() != 0){
          std::list<cedr_app *>::iterator it;
          it = arrived_nonstreaming_apps.begin();
          auto running_app = *it;
          pthread_cancel(running_app->app_pthread);
	        pthread_join(running_app->app_pthread, NULL);
#if defined(ANOMALY_DETECTION)
          emul_time = cedrGetTime(hardware_thread_handle[0].time_per_cycle); 
	        running_app->finish_time = emul_time;
          total_makespan_start = running_app->start_time > total_makespan_start ? total_makespan_start : running_app->start_time;
          total_makespan_end = running_app->finish_time > total_makespan_end ? running_app->finish_time : total_makespan_end;
          struct struct_adfeaturelogging featurelog_obj {};
          strncpy(featurelog_obj.app_name, running_app->app_name, 50);
          featurelog_obj.app_id = running_app->app_id;
          featurelog_obj.arrival = running_app->arrival_time;
          featurelog_obj.start = running_app->start_time;
          featurelog_obj.end = running_app->finish_time;
          featurelog_obj.app_runtime = running_app->finish_time - running_app->start_time;
          featurelog_obj.app_lifetime = running_app->finish_time - running_app->arrival_time;
          featurelog_obj.task_count = running_app->task_count;
          featurelog_obj.max_in_ready_queue = running_app->max_in_ready_queue;
          featurelog_obj.max_in_todo_queue = running_app->max_in_todo_queue;
          featurelog_obj.PE_todo_queue = (unsigned int *)calloc(cedr_config.getTotalResources(), sizeof(unsigned int));
          for (int i = 0; i < cedr_config.getTotalResources(); i++){
            featurelog_obj.PE_todo_queue[i] = running_app->PE_todo_queue[i];
          }
          featurelog_obj.file_size = running_app->file_size;
          //featurelog_obj.min_task = running_app->min_task;
          
          featurelog_obj.min_task = running_app->total_task == 0 ? 0 : running_app->min_task;     
          featurelog_obj.max_task = running_app->total_task == 0 ? 0 : running_app->max_task;
          //printf("running_app->total_task = %d\n",running_app->total_task);
          //printf("running_app->total_task = %d\n",featurelog_obj.min_task);
          //featurelog_obj.mean_task = (float) running_app->total_task / (float) running_app->task_count;
          featurelog_obj.mean_task = running_app->total_task == 0 ? 0.0f : (float) running_app->total_task / (float) running_app->task_count;

          #if defined(USEPERF)
          if (cedr_config.getUsePERF()){
            for (int i=0; i < PERF_EVENT_NUM; i++){
              featurelog_obj.API_perf_counters[i] = running_app->API_perf_counters[i];
              featurelog_obj.APP_perf_counters[i] = running_app->APP_perf_counters[i];
              printf("API_perf_counters[%d] = %llu\n", i, (unsigned long long)featurelog_obj.API_perf_counters[i]);
              printf("APP_perf_counters[%d] = %llu\n", i, (unsigned long long)featurelog_obj.APP_perf_counters[i]);
          ...
          #if defined(CUPTI_LOGGING)
            for (int i=0; i < CUPTI_EVENT_NUM; i++){
              featurelog_obj.APP_cupti_counters[i] = running_app->APP_cupti_counters[i];
            }
          #endif
          ad_feature_log.push_back(featurelog_obj);
#endif
    ...
    for (auto app : arrived_nonstreaming_apps){
      ...
      if (!app->is_running) {
        ...
        pthread_create(&(app->app_pthread), &non_kernel_attr, (void *(*)(void *)) nk_thread_func, (void*) app); //app->main_func_handle
    ...
    if (!ready_queue.empty()) {
      ...
      current_ready_queue_size = ready_queue.size();
      clock_vir0 = cedrGetTime(hardware_thread_handle[0].time_per_cycle);
      performScheduling(cedr_config, ready_queue, hardware_thread_handle, free_resource_count);
      clock_vir1 = cedrGetTime(hardware_thread_handle[0].time_per_cycle);
    }
    pthread_mutex_unlock(&ready_queue_mutex);
    if(current_ready_queue_size!=0 && cedr_config.getLogTraces()){
      struct struct_schedlogging schedlog_obj {};
      schedlog_obj.ready_queue_size = current_ready_queue_size;
      ...
      if(cedr_config.getPeriodicLogs()){
        if(sched_log.size() == cedr_config.getPeriodicLoggingThreshold()){
          {
      ...
      current_ready_queue_size = 0;
    } // Logging enabled

#ifdef ANOMALY_DETECTION
    // TODO: remove below if's 
    if(cedr_config.getLogTraces()){
      if(cedr_config.getPeriodicLogs()){
        struct struct_adfeaturelogging featurelog_obj {};
        emul_time = cedrGetTime(hardware_thread_handle[0].time_per_cycle);
        //printf("emul_time-prev_emul_time  = %lli  \t config_period = %lli \n",emul_time-prev_emul_time ,cedr_config.getLoggingPeriod());

        if(emul_time - prev_emul_time > cedr_config.getLoggingPeriod()){ //1000000000, let's make it 10
          //printf("emul_time = %lli \t prev_emul_time = %lli \t config_period = %lli \n",emul_time,prev_emul_time,cedr_config.getLoggingPeriod());
          if (arrived_nonstreaming_apps.size() != 0){
            for(auto iterator = arrived_nonstreaming_apps.begin(); iterator != arrived_nonstreaming_apps.end(); iterator++){
              auto running_app = *iterator;
              running_app->finish_time = emul_time;
              total_makespan_start = running_app->start_time > total_makespan_start ? total_makespan_start : running_app->start_time;
              total_makespan_end = running_app->finish_time > total_makespan_end ? running_app->finish_time : total_makespan_end;
              struct struct_adfeaturelogging featurelog_obj {};
              ...
              featurelog_obj.max_task = running_app->total_task == 0 ? 0 : running_app->max_task;
              featurelog_obj.mean_task = running_app->total_task == 0 ? 0.0f : (float) running_app->total_task / (float) running_app->task_count;
              featurelog_obj.is_running = 1;
              #if defined(USEPERF)
              if (cedr_config.getUsePERF()){
                unsigned long long perf_counter[PERF_EVENT_NUM];
                /*for (int i=0; i < PERF_EVENT_NUM; i++){
                  ioctl(running_app->perf_fd[i], PERF_EVENT_IOC_DISABLE, 0);
                }*/
                for (int i=0; i < PERF_EVENT_NUM; i++){
                  read(running_app->perf_fd[i], &(perf_counter[i]), sizeof(uint64_t));
                  running_app->APP_perf_counters[i] = perf_counter[i];
                  //close(running_app->perf_fd[i]);
                  
                }
              }
              /*for (int i=0; i < PERF_EVENT_NUM; i++){
                  ioctl(running_app->perf_fd[i], PERF_EVENT_IOC_RESET, 0);
                  ioctl(running_app->perf_fd[i], PERF_EVENT_IOC_ENABLE, 0);
              }*/         
              #endif
              #if defined(USEPERF)
              if (cedr_config.getUsePERF()){
              ...
              #endif
              ad_feature_log_periodic.push_back(featurelog_obj);
            }
            //printf("size of completed_nonstreaming_apps %d .. \n",completed_nonstreaming_apps.size());
            for (auto completed_app = completed_nonstreaming_apps.begin(), completed_app_end = completed_nonstreaming_apps.end(); completed_app!=completed_app_end;) {
              struct struct_applogging applog_obj {};
              auto erase_app = (*completed_app); completed_app++;
              strncpy(applog_obj.app_name, erase_app->app_name, 50);
              /*if(emul_time - erase_app->finish_time > cedr_config.getAppLoggingThreshold() || erase_app->anomaly_or_not){
                continue;
              }*/
              ...
              featurelog_obj.file_size = erase_app->file_size;
              featurelog_obj.min_task = erase_app->total_task == 0 ? 0 : erase_app->min_task; //erase_app->min_task;
              featurelog_obj.max_task = erase_app->total_task == 0 ? 0 : erase_app->max_task;
              featurelog_obj.mean_task = erase_app->total_task == 0 ? 0.0f : (float) erase_app->total_task; //(float) running_app->task_count; (float) erase_app->total_task / (float) erase_app->task_count;
              featurelog_obj.is_running = 0;
              #if defined(USEPERF)
              if (cedr_config.getUsePERF()){
                 unsigned long long perf_counter[PERF_EVENT_NUM];
                /*for (int i=0; i < PERF_EVENT_NUM; i++){
                ...
                for (int i=0; i < PERF_EVENT_NUM; i++){
                  ...
                  read(erase_app->perf_fd[i], &(perf_counter[i]), sizeof(uint64_t));
                   ...
                   featurelog_obj.APP_perf_counters[i] =  erase_app->APP_perf_counters[i]; 
                  //close(erase_app->perf_fd[i]);
                  // you will need to close the PERF files here since the apps are done. 
                }
              }
              #endif              
              #if defined(CUPTI_LOGGING)
            ...
            //printf("cedr_config.getOverwritePeriodicLog() = %d",cedr_config.getOverwritePeriodicLog());
            if(cedr_config.getOverwritePeriodicLog()) ad_features_fp = fopen((log_path + "/ad_features" +".csv").c_str(), "w"); 
            else ad_features_fp =  ad_features_fp = fopen((log_path + "/ad_features" +".csv").c_str(), "a"); 
            //else ad_features_fp = ad_features_fp = fopen((log_path + "/ad_features" + std::to_string(ad_features_log_id) +".csv").c_str(), "w");
            if(ad_features_fp == nullptr){
              LOG_ERROR << "Error outputting anomaly detection feature file!";
            }
            if(ad_features_log_id == 0 ||  cedr_config.getOverwritePeriodicLog()){
              fprintf(ad_features_fp, "app_id,app_name,app_size,app_runtime,app_task_count,max_in_ready_queue,max_in_todo_queue,ref_arrival_time,app_diff_arrival_time,app_lifetime,app_api_min,app_api_max,app_api_mean");
              for (int i = 0; i < cedr_config.getTotalResources(); i++) {
                fprintf(ad_features_fp, ",%s_todo_queue", hardware_thread_handle[i].resource_name.c_str());
              }
              for (int i = 0; i < cedr_config.getTotalResources(); i++) {
              ...
              if (cedr_config.getUsePERF()){
                for (int i=0; i < PERF_EVENT_NUM; i++){
                  fprintf(ad_features_fp, ",API_%s", perf_events_string[i].c_str());
                }
                for (int i=0; i < PERF_EVENT_NUM; i++){
                  fprintf(ad_features_fp, ",APP_%s", perf_events_string[i].c_str());
                }
              }              
              #endif            
              fprintf(ad_features_fp, ",app_is_running");
              #if defined(CUPTI_LOGGING)
                for (int i=0; i < CUPTI_EVENT_NUM; i++){
                  fprintf(ad_features_fp, ",APP_cupti_%s", eventNames[i]);
                }
              #endif
              fprintf(ad_features_fp, ",app_is_running");
              fprintf(ad_features_fp, "\n");
            }  
            std::list<struct struct_adfeaturelogging>::iterator it;
            it = ad_feature_log_periodic.begin();
            uint64_t diff_arrival_time=0;
            uint64_t prev_arrival_time=it->arrival;
            while(it != ad_feature_log_periodic.end()){
              struct struct_adfeaturelogging featurelog_obj = *it;
              diff_arrival_time = (featurelog_obj.arrival > prev_arrival_time) ? featurelog_obj.arrival - prev_arrival_time : prev_arrival_time - featurelog_obj.arrival;
              prev_arrival_time = featurelog_obj.arrival;
              fprintf(ad_features_fp, "%d,%s,%llu,%" PRIu64 ",%u,%u,%u,%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%f",
                                      featurelog_obj.app_id, featurelog_obj.app_name, featurelog_obj.file_size, featurelog_obj.app_runtime, featurelog_obj.task_count, featurelog_obj.max_in_ready_queue, featurelog_obj.max_in_todo_queue, featurelog_obj.arrival, diff_arrival_time, featurelog_obj.app_lifetime, featurelog_obj.min_task, featurelog_obj.max_task, featurelog_obj.mean_task);
              for (int i = 0; i < cedr_config.getTotalResources(); i++){
                fprintf(ad_features_fp, ",%u", featurelog_obj.PE_todo_queue[i]);
              }
              for (int i = 0; i < cedr_config.getTotalResources(); i++){
                fprintf(ad_features_fp, ",%f", hardware_thread_handle[i].total_runtime / (float) (total_makespan_end - total_makespan_start) );
              }
    
            #if defined(USEPERF)
            if (cedr_config.getUsePERF()){
              for (int i=0; i < PERF_EVENT_NUM; i++){
                fprintf(ad_features_fp, ",%llu", featurelog_obj.API_perf_counters[i]);
              }
              for (int i=0; i < PERF_EVENT_NUM; i++){
                fprintf(ad_features_fp, ",%llu", featurelog_obj.APP_perf_counters[i]);
              }
            }
            #endif                    
            #if defined(CUPTI_LOGGING)
              for (int i=0; i < CUPTI_EVENT_NUM; i++){
                fprintf(ad_features_fp, ",%llu", featurelog_obj.APP_cupti_counters[i]);
              }
            #endif
              fprintf(ad_features_fp, ",%lu",featurelog_obj.is_running);
              //printf("APP IS RUNNING = %lu",featurelog_obj.is_running);
              fprintf(ad_features_fp, "\n");
              it = ad_feature_log_periodic.erase(it);
            }
            //system("cat log_dir/experiment0/ad_features.csv | wc -l"); // this was used to print the number of the lines
            ad_features_log_id++;
            if(cedr_config.getOverwritePeriodicLog()) chmod((log_path + "/ad_features" +".csv").c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);  
            else chmod((log_path + "/ad_features" + std::to_string(ad_features_log_id) +".csv").c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH); 
            fclose(ad_features_fp);

          }
          prev_emul_time = emul_time;
          //printf("Exited the if\n");
        }
      }
    }
    ...
    // Write timing trace logs if periodic logging is enabled
    if(cedr_config.getLogTraces()){
      if(cedr_config.getPeriodicLogs()){
        writeTimingTrace(trace_fp, cedr_config, hardware_thread_handle, true, &earliestStart, &latestFinish);
      } // Periodic logging enabled
    } // Logging enabled
    ...
    for (auto poison_task = poison_queue.begin(), poison_task_end = poison_queue.end(); poison_task != poison_task_end;){
      ...
      if(cedr_config.getLogTraces()){
        ...
        if(cedr_config.getPeriodicLogs()){
          if(completed_nonstreaming_apps.size() == cedr_config.getPeriodicLoggingThreshold()){
#if defined(ANOMALY_DETECTION)
            writeApplicationTrace(apptrace_fp, cedr_config,  hardware_thread_handle, &completed_nonstreaming_apps, &earliestStart, &latestFinish, trace_fp, &total_makespan_start, &total_makespan_end, &ad_feature_log);
#else
            writeApplicationTrace(apptrace_fp, cedr_config,  hardware_thread_handle, &completed_nonstreaming_apps, &earliestStart, &latestFinish, trace_fp);
#endif
          }
        }
      }
    ...
    std::vector<int>anomaly_app_ids;
    std::string line;
    int id;
    std::ifstream file_handler("anomaly_ids.txt");
    if (file_handler.is_open()) {
      while (file_handler>>id) {
      anomaly_app_ids.push_back(id);
      }
      file_handler.close();
    }

    if (arrived_nonstreaming_apps.size() != 0){
      for (auto anomaly_id : anomaly_app_ids){
        for (auto running_app : arrived_nonstreaming_apps){
          if(anomaly_id == running_app->app_id){
            running_app->anomaly_or_not = true;
          }
        }
      }
    }
  ...
  if (arrived_nonstreaming_apps.size() != 0){
    //#for (auto iterator = arrived_nonstreaming_apps.begin(); iterator != arrived_nonstreaming_apps.end(); iterator++){
    for (int iter = 0; iter < arrived_nonstreaming_apps.size();){
      auto front = arrived_nonstreaming_apps.begin();
      std::advance(front, iter);
      auto running_app = *front;
      if(running_app->anomaly_or_not){
        bool kill = true;
        for (int i = 0; i < cedr_config.getTotalResources(); i++) {
          if(hardware_thread_handle[i].task != nullptr){
            pthread_mutex_lock(&(hardware_thread_handle[i].resource_mutex));
            auto app = hardware_thread_handle[i].task->app_pnt;
            if(hardware_thread_handle[i].task->app_pnt->app_id == running_app->app_id){
              kill = false;
            }
            pthread_mutex_unlock(&(hardware_thread_handle[i].resource_mutex));
          }
        ...
        if(kill){
          printf("APP is anomaly, killing %s with id:%d\n", running_app->app_name, running_app->app_id);
          pthread_cancel(running_app->app_pthread);
          pthread_join(running_app->app_pthread, NULL);
          arrived_nonstreaming_apps.remove(running_app);
          completed_nonstreaming_apps.push_back(running_app);
          #if defined(USEPERF)
          if (cedr_config.getUsePERF()){
            unsigned long long perf_counter[PERF_EVENT_NUM];
            for (int i=0; i < PERF_EVENT_NUM; i++){
  ...
  #if defined(TASK_LOGGING) // timing_trace.log and  schedule_trace.csv can be very large! we do not need it for anomaly detection. 
  {
    ...
    if(cedr_config.getPeriodicLogs() == false){
      trace_fp = fopen((log_path + "/timing_trace.log").c_str(), "w");
    }
    if (trace_fp == nullptr) {
      LOG_ERROR << "Error opening output trace file!";
    } else {
...
/*#if defined(ANOMALY_DETECTION)
    if(!cedr_config.getPeriodicLogs()){
      ad_features_fp = fopen((log_path + "/ad_features.csv").c_str(), "w");
      if(ad_features_fp == nullptr){
        LOG_ERROR << "Error outputting anomaly detection feature file!";
      }
      fprintf(ad_features_fp, "app_id,app_name,app_size,app_runtime,app_task_count,max_in_ready_queue,max_in_todo_queue,ref_arrival_time,app_diff_arrival_time,app_lifetime,app_api_min,app_api_max,app_api_mean");
    ...
    std::list<struct struct_adfeaturelogging>::iterator it;
    it = ad_feature_log.begin();
    uint64_t diff_arrival_time=0;
    uint64_t prev_arrival_time=it->arrival;
    while(it != ad_feature_log.end()){
      struct struct_adfeaturelogging featurelog_obj = *it;
      diff_arrival_time = (featurelog_obj.arrival > prev_arrival_time) ? featurelog_obj.arrival - prev_arrival_time : prev_arrival_time - featurelog_obj.arrival;
      prev_arrival_time = featurelog_obj.arrival;
      fprintf(ad_features_fp, "%d,%s,%llu,%" PRIu64 ",%u,%u,%u,%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%f",
      ...
      fprintf(ad_features_fp, "\n");
      it = ad_feature_log.erase(it);
    }
    fclose(ad_features_fp);
    chmod((log_path + "/ad_features.csv").c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
#endif // ANOMALY_DETECTION endif*/

...
void _fft_hs(cplx buf[], cplx out[], int n, int step) {
    ...
    if (step < n) {
	    ...
	    cplx t = exp(-I * PI * (double)i / (double)n) * out[i + step];
	    buf[i / 2]     = out[i] + t;
	    buf[(i + n)/2] = out[i] - t;
        }
    }
...
void _ifft_hs(cplx buf[], cplx out[], int n, int step) {

  int i;

  if (step < n) {
    _ifft_hs(out, buf, n, step * 2);
    _ifft_hs(out + step, buf + step, n, step * 2);
    for (i = 0; i < n; i += 2 * step) {
      cplx t = exp(I * PI * (double)i / (double)n) * out[i + step];
      buf[i / 2]     = out[i] + t;
      buf[(i + n)/2] = out[i] - t;
...
void ifft_hs(int fft_id, comp_t fdata[], int n, int hw_fft_busy) {
    ...
    if (hw_fft_busy == 1) {
        ...
        for (i = 0; i < n; i++) {
            //fdata[i].real = (float)creal(buf[i]);
            //fdata[i].imag = (float)cimag(buf[i]);;
            fdata[i].real = (float)buf[i].real();
            fdata[i].imag = (float)buf[i].imag();;
        }

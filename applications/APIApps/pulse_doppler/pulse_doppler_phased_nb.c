...
void xcorr(double *, double *, size_t, double *, size_t);
void swap(double *, double *);  
void fftshift(double *, double);
                       
/* Function Definitions */                
void xcorr(double *x, double *y, size_t n_samp, double *corr, size_t m) {
  size_t len;    
  len = 2 * n_samp;  
  double *c = malloc(m * 2 * len * sizeof(double));
  double *d = malloc(m * 2 * len * sizeof(double));
                                          
  size_t x_count = 0;
  size_t y_count = 0;

  double *z = malloc(m * 2 * (n_samp) * sizeof(double));    
...
for (int k = 0; k < m; k++) {
  ...
  memcpy(c + k*(2*len) + 2 * (n_samp - 1), x, 2 * n_samp * sizeof(double));   
  c[k*(2*len) + 2 * len - 2] = 0;
  c[k*(2*len) + 2 * len - 1] = 0;
  memcpy(d + k*(2*len), y, 2 * n_samp * sizeof(double));   
  memcpy(d + k*(2*len) + 2 * n_samp, z + k*len, 2 * (n_samp) * sizeof(double)); 
}  
  double *X1 = malloc(m * 2 * len * sizeof(double));        
  double *X2 = malloc(m * 2 * len * sizeof(double));        
  double *corr_freq = malloc(m * 2 * len * sizeof(double));
 
  // gsl_fft(c, X1, len) and gsl_fft(d, X2, len) (independent);   
  // lol scope
  {

    dash_cmplx_flt_type **fft_inp_x1 = (dash_cmplx_flt_type**) malloc(m * sizeof(dash_cmplx_flt_type*));
    dash_cmplx_flt_type **fft_out_x1 = (dash_cmplx_flt_type**) malloc(m * sizeof(dash_cmplx_flt_type*));
    dash_cmplx_flt_type **fft_inp_x2 = (dash_cmplx_flt_type**) malloc(m * sizeof(dash_cmplx_flt_type*));
    dash_cmplx_flt_type **fft_out_x2 = (dash_cmplx_flt_type**) malloc(m * sizeof(dash_cmplx_flt_type*));

    bool is_fwd = true;
for (int k = 0; k < m; k++) {
    fft_inp_x1[k] = (dash_cmplx_flt_type*) malloc(len * sizeof(dash_cmplx_flt_type));
    fft_out_x1[k] = (dash_cmplx_flt_type*) malloc(len * sizeof(dash_cmplx_flt_type));
    fft_inp_x2[k] = (dash_cmplx_flt_type*) malloc(len * sizeof(dash_cmplx_flt_type));
    fft_out_x2[k] = (dash_cmplx_flt_type*) malloc(len * sizeof(dash_cmplx_flt_type));

    for (size_t i = 0; i < len; i++) {
      fft_inp_x1[k][i].re = (dash_re_flt_type) c[k*2*len + 2*i];
      fft_inp_x1[k][i].im = (dash_re_flt_type) c[k*2*len + 2*i+1];
      fft_inp_x2[k][i].re = (dash_re_flt_type) d[k*2*len + 2*i];
      fft_inp_x2[k][i].im = (dash_re_flt_type) d[k*2*len + 2*i+1];
    }
}
...
for (int k = 0; k < m; k++) {
    ...
    DASH_FFT_flt_nb(&fft_inp_x1[k], &fft_out_x1[k], &len, &is_fwd, &barrier);
    DASH_FFT_flt_nb(&fft_inp_x2[k], &fft_out_x2[k], &len, &is_fwd, &barrier);
}

    pthread_mutex_lock(barrier.mutex);
    if (completion_ctr != completion) {
...
for (int k = 0; k < m; k++) {
    for (size_t i = 0; i < len; i++) {
      X1[k*2*len + 2*i] = (double) fft_out_x1[k][i].re;
      X1[k*2*len + 2*i+1] = (double) fft_out_x1[k][i].im;
      X2[k*2*len + 2*i] = (double) fft_out_x2[k][i].re;
      X2[k*2*len + 2*i+1] = (double) fft_out_x2[k][i].im;
    }
}
    free(fft_inp_x1);
    free(fft_out_x1);
...
for (int k = 0; k < m; k++) {
  ...
  // lol scope
  {
    ...
    bool is_fwd = false;
for (int k = 0; k < m; k++) {
    fft_inp[k] = (dash_cmplx_flt_type*) malloc(len * sizeof(dash_cmplx_flt_type));
    fft_out[k] = (dash_cmplx_flt_type*) malloc(len * sizeof(dash_cmplx_flt_type));

    for (size_t i = 0; i < len; i++) {
      fft_inp[k][i].re = (dash_re_flt_type) corr_freq[k*2*len + 2*i];
      fft_inp[k][i].im = (dash_re_flt_type) corr_freq[k*2*len + 2*i+1];
    }
}
...
for (int k = 0; k < m; k++) {
    DASH_FFT_flt_nb(&fft_inp[k], &fft_out[k], &len, &is_fwd, &barrier);
}
    pthread_mutex_lock(barrier.mutex);
    if (completion_ctr != completion) {
      pthread_cond_wait(barrier.cond, barrier.mutex);
...
for (int k = 0; k < m; k++) {
    for (size_t i = 0; i < len; i++) {
      corr[k*2*len + 2*i] = (double) fft_out[k][i].re;
      corr[k*2*len + 2*i+1] = (double) fft_out[k][i].im;
    }
}
    free(fft_inp);
    free(fft_out);
...
int main(int argc, char *argv[]) {
  ...
  double *mf = malloc((2 * n_samples) * m * 2 *
                      sizeof(double)); // build a 2D array for the output of the
                                       // matched filter
  double *p =
      malloc(m * 2 * n_samples * sizeof(double)); // array for pulse with noise
  double *pulse =
      malloc(2 * n_samples * sizeof(double)); // array for the original pulse
  double *corr =
      malloc(m * 2 * (2 * n_samples) *
...
//    }
    ...
    xcorr(p, pulse, n_samples, corr, m);

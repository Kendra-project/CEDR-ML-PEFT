...
void xcorr(double *x, double *y, size_t n_samp, double *corr) {
  ...
  dash_cmplx_flt_type *X1 = malloc(len * sizeof(dash_cmplx_flt_type));        
  dash_cmplx_flt_type *X2 = malloc(len * sizeof(dash_cmplx_flt_type));        
  dash_cmplx_flt_type *corr_freq = malloc(len * sizeof(dash_cmplx_flt_type));
  ...
  // lol scope
  {
    ...
    DASH_FFT_flt_nb(&fft_inp_x1, &X1, &len, &is_fwd, &barrier);
    DASH_FFT_flt_nb(&fft_inp_x2, &X2, &len, &is_fwd, &barrier);
  ...
  // lol scope
  {
    ...
    DASH_FFT_flt_nb(&corr_freq, &fft_out, &len, &is_fwd, &barrier);

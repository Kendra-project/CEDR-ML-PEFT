...
void grayscale_conversion(int16_t *img, int height, int width, int depth, int16_t *return_array){
...
void gaussian_blur(int16_t *img, int kernel_size, int height, int width, int16_t *return_array){
    double r, g, p, s, e;
    float sum = 0;
...
void sobel(int16_t *gx, int16_t *gy, int height, int width, int lower_threshold, int upper_threshold, int16_t *return_array){
...
// width --> image width
// Output: return_array --> edge detected frame
void roi(int height, int width, int16_t *return_array){
...
std::vector<Line> hough_transform(int16_t *img, int height, int width, float rho, float theta, int threshold, int line_length, int line_gap){
...
void draw_line(int x1, int y1, int x2, int y2, int width, int dimension, int color, int16_t *return_array){
    int dx = x2 - x1;
    int dy = y2 - y1;
    int yi = 1;
    if(dy < 0){
...
void draw_circle(int *center, int r, int color, int height, int width, int depth, int16_t *return_array){
    int r_sq = r * r;
    int x_diff, y_diff;
    for(int y = 0; y < height; y++){
        for(int x = 0; x < width; x++){
...
void draw_lane(std::vector<Line> lines, int height, int width, int16_t * return_array, int *avg_left_return, int *avg_right_return){
...
void save_img(int16_t* img, int height, int width, std::string name){
...
int main(){
    ...
    int16_t *temp_arr = (int16_t *) malloc (sizeof (int16_t) * (size * 3));
    int16_t *img = (int16_t *) malloc (sizeof (int16_t) * (size * 3));
    int16_t *grayscale_img = (int16_t *) malloc (sizeof (int16_t) * size);
    int16_t *blurred_img = (int16_t *) malloc (sizeof (int16_t) * size);
    int16_t *blurred_img_before_canny = (int16_t *) malloc (sizeof (int16_t) * size);
    int16_t *gx_canny = (int16_t *) malloc (sizeof (int16_t) * size);
    int16_t *gy_canny = (int16_t *) malloc (sizeof (int16_t) * size);
    int16_t *sobel_img = (int16_t *) malloc (sizeof (int16_t) * size);
    int16_t *lanes = (int16_t *) malloc (sizeof (int16_t) * size);

    for (int i = 0; i<(height * width)*3; i++){
       img[i] = (int16_t)rgb_imag[i];
    }

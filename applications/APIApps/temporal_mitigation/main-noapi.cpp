...
int main(int argc, char *argv[]) {
  ...
  // lol scope
  {
    ...
    //DASH_GEMM_flt(A_api, B_api, C_api, N, M, N);
		...
		DASH_GEMM_flt_cpu(&A_api, &B_api, &C_api, &N_noapi, &M_noapi, &N_noapi);
  ...
  // lol scope
  {
    ...
    //DASH_GEMM_flt(A_api, B_api, C_api, N, N, N);
		DASH_GEMM_flt_cpu(&A_api, &B_api, &C_api, &N_noapi, &N_noapi,&N_noapi);
    for (size_t i = 0; i < N * N; i++) {
  ...
  // lol scope
  {
    ...
    //DASH_GEMM_flt(A_api, B_api, C_api, N, N, M);
		DASH_GEMM_flt_cpu(&A_api, &B_api, &C_api, &N_noapi, &N_noapi,&M_noapi);
    for (size_t i = 0; i < N * N; i++) {

...
int main(void)
{
    ...
    double *c, *d;
    dash_cmplx_flt_type *X1, *X2, *corr_freq;
    ...
    X1 = malloc(len * sizeof(dash_cmplx_flt_type));
    X2 = malloc(len * sizeof(dash_cmplx_flt_type));
    corr_freq = malloc(len * sizeof(dash_cmplx_flt_type));
    ...
    // lol scope
    {
      ...
      DASH_FFT_flt(fft_inp, X1, len, true);
      free(fft_inp);
    }
    // lol scope
    {
      ...
      DASH_FFT_flt(fft_inp, X2, len, true);
      free(fft_inp);
    }
    for (size_t i = 0; i < len; i++) {
      X2[i].im = -X2[i].im;
    ...
    // lol scope
    {
      ...
      DASH_FFT_flt(corr_freq, fft_out, len, false);
    ...
    printf(" ZIP Radar correlator complete; lag Value is: %lf\n", lag);

...
int main(void) {
  ...
  DASH_FFT_flt_cpu(&g[0], &g2[0], &fast, &forwardTrans);

{
  ...
  for (i = 0; i < Nslow; i++) {
    DASH_FFT_flt_cpu(&s0[i], &fft_out_0[i], &fast, &forwardTrans);
...

{
  ...
  for (i = 0; i < Nslow; i++) {
    DASH_ZIP_flt_cpu(&fft_out_0[i], &g2[0], &fft_inp_1[i], &fast, &op);
/*
    for (j = 0; j < Nfast; j += 1) {
...

{
  ...
  for (i = 0; i < Nslow; i++) {
    DASH_FFT_flt_cpu(&fft_inp_1[i], &fft_out_1[i], &fast, &forwardTrans);
  }
...

{
  ...
  for (i = 0; i < Nfast; i++) {
    DASH_FFT_flt_cpu(&fft_inp_3[i], &S1[i], &slow, &forwardTrans);
  }
...

{
  ...
  // Azimuth Compression
  for (i = 0; i < Nfast; i++) {
    DASH_ZIP_flt_cpu(&S1[i], &H[0], &fft_inp_2[i], &slow, &op);
/*
    for (j = 0; j < Nfast; j += 1) {
...

{
  ...
  for (i = 0; i < Nfast; i++) {
    DASH_FFT_flt_cpu(&fft_inp_2[i], &fft_out_2[i], &slow, &forwardTrans);
  }

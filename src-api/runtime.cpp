...
void launchDaemonRuntime(ConfigManager &cedr_config, pthread_t *resource_handle, worker_thread *hardware_thread_handle, pthread_mutex_t *resource_mutex) {
  ...
  unsigned long long total_makespan_start=0;
  unsigned long long total_makespan_end=0;
...
*/      }
      ...
      for (auto task : poison_queue){
        ...
        if (total_makespan_start > applog_obj.start || total_makespan_start == 0){
          total_makespan_start = applog_obj.start;
        }
				total_makespan_end = applog_obj.end;
        applog_obj.app_runtime = applog_obj.end - applog_obj.start;
        applog_obj.app_lifetime = applog_obj.end - applog_obj.arrival;
        applog_obj.task_count = task->app_pnt->task_count;
        applog_obj.max_in_ready_queue = task->app_pnt->max_in_ready_queue;
        applog_obj.file_size = task->app_pnt->file_size;
        applog_obj.max_in_todo_queue = task->app_pnt->max_in_todo_queue;
        applog_obj.PE_todo_queue = (unsigned int *)calloc(cedr_config.getTotalResources(), sizeof(unsigned int));
        for (int i = 0; i < cedr_config.getTotalResources(); i++){
          applog_obj.PE_todo_queue[i] = task->app_pnt->PE_todo_queue[i];
        }
#if defined(USEPERF)
      if (cedr_config.getUsePERF()){
        for (int i=0; i < PERF_EVENT_NUM; i++){
          applog_obj.API_perf_counters[i] = task->app_pnt->API_perf_counters[i];
          applog_obj.APP_perf_counters[i] = task->app_pnt->APP_perf_counters[i];
        }
      }
#endif
      ...
      if (arrived_nonstreaming_apps.size() != 0){
        ...
        while (arrived_nonstreaming_apps.size() != 0){//(auto running_app : arrived_nonstreaming_apps){
        ...
        if (cedr_config.getUsePERF()){
...
#endif
          strncpy(applog_obj.app_name, running_app->app_name, 50);
          applog_obj.app_id = running_app->app_id;
          applog_obj.arrival = running_app->arrival_time;
          applog_obj.start = running_app->start_time;
          applog_obj.end = (real_current_time.tv_sec * SEC2NANOSEC + real_current_time.tv_nsec);
          if (total_makespan_start > applog_obj.start || total_makespan_start == 0){
            total_makespan_start = applog_obj.start;
          }
          ...
          applog_obj.app_runtime = applog_obj.end - applog_obj.start;
          applog_obj.app_lifetime = applog_obj.end - applog_obj.arrival;
          applog_obj.task_count = running_app->task_count;
          applog_obj.max_in_ready_queue = running_app->max_in_ready_queue;
          applog_obj.file_size = running_app->file_size;
          applog_obj.max_in_todo_queue = running_app->max_in_todo_queue;
          applog_obj.PE_todo_queue = (unsigned int *)calloc(cedr_config.getTotalResources(), sizeof(unsigned int));
          for (int i = 0; i < cedr_config.getTotalResources(); i++){
            applog_obj.PE_todo_queue[i] = running_app->PE_todo_queue[i];
          }
#if defined(USEPERF)
          if (cedr_config.getUsePERF()){
            for (int i=0; i < PERF_EVENT_NUM; i++){
              applog_obj.API_perf_counters[i] = running_app->API_perf_counters[i];
              applog_obj.APP_perf_counters[i] = running_app->APP_perf_counters[i];
            }
          }
#endif
          app_log.push_back(applog_obj);
	...
	// App runtime logging
        ...
        if (cedr_config.getUsePERF()){
          unsigned long long perf_counter[PERF_EVENT_NUM];
          for (int i=0; i < PERF_EVENT_NUM; i++){
            ioctl(erased_app->perf_fd[i], PERF_EVENT_IOC_DISABLE, 0);
          }
          for (int i=0; i < PERF_EVENT_NUM; i++){
            read(erased_app->perf_fd[i], &(perf_counter[i]), sizeof(uint64_t));
            erased_app->APP_perf_counters[i] = perf_counter[i];
            close(erased_app->perf_fd[i]);
          }
        }
#endif
        ...
        applog_obj.app_runtime = applog_obj.end - applog_obj.start;
        applog_obj.app_lifetime = applog_obj.end - applog_obj.arrival;
        applog_obj.task_count = erased_app->task_count;
        applog_obj.max_in_ready_queue = erased_app->max_in_ready_queue;
	      applog_obj.file_size = erased_app->file_size;
        applog_obj.max_in_todo_queue = erased_app->max_in_todo_queue;
        applog_obj.PE_todo_queue = (unsigned int *)calloc(cedr_config.getTotalResources(), sizeof(unsigned int));
        for (int i = 0; i < cedr_config.getTotalResources(); i++){
          applog_obj.PE_todo_queue[i] = erased_app->PE_todo_queue[i];
        }
#if defined(USEPERF)
        if (cedr_config.getUsePERF()){
          for (int i=0; i < PERF_EVENT_NUM; i++){
            applog_obj.API_perf_counters[i] = erased_app->API_perf_counters[i];
            applog_obj.APP_perf_counters[i] = erased_app->APP_perf_counters[i];
          }
        }
#endif
        app_log.push_back(applog_obj);
  ...
  std::map<int, std::vector<long long>> app_task_times; // Map to store task times for each app_id
  {
   ...
   /*FILE *trace_fp = fopen((log_path + "/timing_trace.log").c_str(), "w");
    if (trace_fp == nullptr) {
      ...
      while (it != stream_timing_log.end()) {
        ...
        /*fprintf(trace_fp,
				...
				// Find the index of the app_id in api_data_vector
        int app_id_index = task.app_id;
        // Calculate statistics for the current task
        long long task_time = e0 - s0;
				//std::cout << "app_id_index = " << app_id_index << std::endl;
				//std::cout << "task_time = " << task_time << std::endl;			
        // Update the statistics in api_data_vector
				app_task_times[app_id_index].push_back(task_time);
    ...
    /* Print the task times for each app_id
    for (const auto& pair : app_task_times) {
        int app_id = pair.first;
        const std::vector<long long>& task_times = pair.second;
    ...
    int app_id_index = 0; 
    auto min_it = std::min_element(app_task_times[app_id_index].begin(), app_task_times[app_id_index].end());
    long long min_value = (min_it != app_task_times[app_id_index].end()) ? *min_it : 0;

    // Calculate the maximum value
    auto max_it = std::max_element(app_task_times[app_id_index].begin(), app_task_times[app_id_index].end());
    long long max_value = (max_it != app_task_times[app_id_index].end()) ? *max_it : 0;
    // Calculate the mean value
    double mean_value = 0.0;
    if (!app_task_times[app_id_index].empty()) {
        mean_value = static_cast<double>(std::accumulate(app_task_times[app_id_index].begin(), app_task_times[app_id_index].end(), 0ll)) / app_task_times[app_id_index].size();
    }   cancle
    /* Print the statistics
    std::cout << "App ID: " << app_id_index << std::endl;
    std::cout << "Minimum Task Time: " << min_value << std::endl;
    std::cout << "Maximum Task Time: " << max_value << std::endl;
    std::cout << "Mean Task Time: " << mean_value << std::endl;  */
  ...
  // App Runtime log capturing
  {
    ...
    if (apptrace_fp == nullptr){
      LOG_ERROR << "Error outputting app runtime log file!";
    }
    if (ad_features_fp == nullptr){
      ...
      //long long min_task,max_task,mean_task = 0;
			long long prev_arrival = 0;
			long long diff_arrival_time = 0;
			bool flag = false;
      while (it != app_log.end()){
        ...
        if(!flag) {
        	...
        	flag = true;
 				...
 				else diff_arrival_time = applog_element.arrival - prev_arrival;
   			prev_arrival = applog_element.arrival;


   			int app_id_index = applog_element.app_id;
				auto min_it = std::min_element(app_task_times[app_id_index].begin(), app_task_times[app_id_index].end());
				long long min_task = (min_it != app_task_times[app_id_index].end()) ? *min_it : 0;

				// Calculate the maximum value
				auto max_it = std::max_element(app_task_times[app_id_index].begin(), app_task_times[app_id_index].end());
				long long max_task = (max_it != app_task_times[app_id_index].end()) ? *max_it : 0;

				// Calculate the mean value

				long long mean_task = 0;
				if (!app_task_times[app_id_index].empty()) {
				    mean_task = std::accumulate(app_task_times[app_id_index].begin(), app_task_times[app_id_index].end(), 0ll) / app_task_times[app_id_index].size();
				}         
      	fprintf(ad_features_fp, "%d,%s,%llu,%lld,%u,%u,%u,%lld,%lld,%lld,%lld,%lld,%lld",
			  applog_element.app_id, applog_element.app_name, applog_element.file_size, applog_element.app_runtime, applog_element.task_count, applog_element.max_in_ready_queue, applog_element.max_in_todo_queue,applog_element.arrival,diff_arrival_time,applog_element.app_lifetime,min_task,max_task,mean_task);
        for (int i = 0; i < cedr_config.getTotalResources(); i++){
      	  fprintf(ad_features_fp, ",%u", applog_element.PE_todo_queue[i]);
        }
        for (int i = 0; i < cedr_config.getTotalResources(); i++) {
        ...
        if (cedr_config.getUsePERF()){
          for (int i=0; i < PERF_EVENT_NUM; i++){
            fprintf(ad_features_fp, ",%llu", applog_element.API_perf_counters[i]);
          }
          for (int i=0; i < PERF_EVENT_NUM; i++){
            fprintf(ad_features_fp, ",%llu", applog_element.APP_perf_counters[i]);
          }
        }
#endif
      	fprintf(ad_features_fp, "\n");  	

        fprintf(apptrace_fp, "app_id: %d, app_name: %s, ref_arrival_time: %lld, ref_start_time: %lld, ref_end_time: %lld, app_runtime: %lld, app_lifetime: %lld, app_api_min: %lld, app_api_max: %lld, app_api_mean: %lld \n", applog_element.app_id, applog_element.app_name, applog_element.arrival, applog_element.start, applog_element.end, applog_element.app_runtime, applog_element.app_lifetime
        ,min_task,max_task,mean_task);
        it = app_log.erase(it);
      }

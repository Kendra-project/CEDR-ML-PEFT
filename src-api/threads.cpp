...
void *hardware_thread(void *ptr) {
  ...
  unsigned long long /*first_busy = 0,*/ start = 0, last_avail = 0, total_runtime = 0;
    ...
    if ((!worker_thread->todo_task_dequeue.empty()) && worker_thread->resource_state != 3) {
      ...
      *reinterpret_cast<void **>(&run_func) = task_run_func;
      ...
      if (args.size() >= MAX_ARGS) {
        ...
        start = (worker_thread->task->start.tv_sec * SEC2NANOSEC) + (worker_thread->task->start.tv_nsec);
//        if (first_busy == 0) {
//          first_busy = start;
...
//        }
        ...
        */
        ...
        total_runtime += last_avail - start;
      }
#if defined(USEPAPI)
      if (cedr_config->getUsePAPI() && PAPI_is_initialized()) {
      ...
      if (worker_thread->resource_state == 3) {
        worker_thread->total_runtime = total_runtime;
        pthread_mutex_unlock(thread_lock);
        break;
      }
      pthread_mutex_unlock(thread_lock);

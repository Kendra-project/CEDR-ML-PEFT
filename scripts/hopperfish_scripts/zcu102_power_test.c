	...
	for (int j = 0; j < iterations; j++) {
		...
		while(1) {
			...
			if(inas[counter].last) {
				...
				if (j == 0){
				fprintf(sav_ptr, "PS Power (mW), PL Power (mW), MGT Power (mW), Total Power (mW)");
				if(display==1){
					printf("PS Power (mW), PL Power (mW), MGT Power (mW), Total Power (mW)\n");
				}
				fprintf(sav_ptr, "\n");
				}

				pspower = (double) (inas[VCCPSINTFP].voltage*inas[VCCPSINTFP].current+
				...
				fprintf(sav_ptr, " %.7f,", pspower);
				if(display==1){
					printf(" %.7f,", pspower);
				}
				plpower = (double) (inas[VCCINT].voltage*inas[VCCINT].current+
				...
				fprintf(sav_ptr, " %.7f,", plpower);
				if(display==1){
					printf(" %.7f,", plpower);
				}
				mgtpower = (double) (inas[MGTRAVCC].voltage*inas[MGTRAVCC].current+
				...
				fprintf(sav_ptr, " %.7f,", mgtpower);
				if(display==1){
					printf(" %.7f,", mgtpower);
				}

				fprintf(sav_ptr, " %.7f", mgtpower+plpower+pspower);
				if(display==1){
...
int main(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv, "t:o:vdn:l")) != -1) {
		...
		switch (opt) {
			...
			case 't':
				printf("Running with sleep @ microseconds%d\n", atoi(optarg));

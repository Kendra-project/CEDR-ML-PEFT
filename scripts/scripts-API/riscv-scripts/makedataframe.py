...
def resource_util(FILENAME):
    ...
    with open(FILENAME, newline='') as fp:
        ...
        stop_dict = {}
        exectime_dict = {}
        corelist = [' cpu1', ' cpu2', ' fft1',' fft2',' fft3']  # Edit here
...
def generate_argparser():
    ...
    parser.add_argument('-t', dest='trial', default=5, help='Number of trials to use')
    parser.add_argument('-r', dest='injectionRateCount', default=15, help='Number of injection rates used for the tests')
    return parser
...
if __name__ == '__main__':
    ...
    if WORKLOAD == 'HIGH':
        #INJ_RATES=[10, 30, 50, 80, 100, 400, 700, 1000, 1300, 1600, 1900, 2000]
        #WF_PERIODS=[101270, 33757, 20254, 12659, 10127, 2532, 1447, 1013, 779, 633, 533, 506]
        #PD_PERIODS=[101270, 33757, 20254, 12659, 10127, 2532, 1447, 1013, 779, 633, 533, 506]

        #INJ_RATES=[1000,1500,2000,3000,4000,5000,10000,15000,20000,30000,40000,50000]
        #PD_PERIODS=[868750,579167,434375,289583,217188,173750,86875,57917,43438,28958,21719,17375]
        #WF_PERIODS=[434375,289583,217188,144792,108594,86875,43438,28958,21719,14479,10859,8688]

        #INJ_RATES=[1000,2000,4000,10000, 20000,50000]
        #PD_PERIODS=[868750,434375,217188,86875,43438,17375]
        #WF_PERIODS=[868750,434375,217188,86875,43438,17375]

        INJ_RATES=[0.1,0.2,0.3,0.5,0.75,1,1.5,2,3,4,5,10,20,40,50]
        PD_PERIODS=[8687500,4343750,2895833,1737500,115833,868750,579167,434375,289583,217188,173750,86875,43438,21719,17375]
        WF_PERIODS=[8687500,4343750,2895833,1737500,1158333,868750,579167,434375,289583,217188,173750,86875,43438,21719,17375]
                    ...
                    for c in [CPUS-1]:
                        for sched in SCHEDS:
                            for i in range(INJ_COUNT):
                                row_to_write = []
                                resPoolComb = 'C'+str(c+1)+'+F'+str(f) # Edit here
                                ...
                                for t in range(TRIALS):
                                    #file_to_parse = BASEDIR+'/'+WORKLOAD+'/trial_'+str(t+1)+'/c'+str(c+1)+'_f'+str(f)+'-'+sched+'-p'+str(PD_PERIODS[i])+','+str(WF_PERIODS[i])+','+str(WF_PERIODS[i])+'/timing_trace.log' # Edit here
                                    #sched_file_to_parse = BASEDIR+'/'+WORKLOAD+'/trial_'+str(t+1)+'/c'+str(c+1)+'_f'+str(f)+'-'+sched+'-p'+str(PD_PERIODS[i])+','+str(WF_PERIODS[i])+','+str(WF_PERIODS[i])+'/schedule_trace.log' # Edit here
                                    #apprun_file_to_parse= BASEDIR+'/'+WORKLOAD+'/trial_'+str(t+1)+'/c'+str(c+1)+'_f'+str(f)+'-'+sched+'-p'+str(PD_PERIODS[i])+','+str(WF_PERIODS[i])+','+str(WF_PERIODS[i])+'/appruntime_trace.log'
                                    
                                    file_to_parse = BASEDIR+'/'+WORKLOAD+'/trial_'+str(t+1)+'/c'+str(c+1)+'_f'+str(f)+'-'+sched+'-p'+str(PD_PERIODS[i])+','+str(WF_PERIODS[i])+'/timing_trace.log' # Edit here
                                    sched_file_to_parse = BASEDIR+'/'+WORKLOAD+'/trial_'+str(t+1)+'/c'+str(c+1)+'_f'+str(f)+'-'+sched+'-p'+str(PD_PERIODS[i])+','+str(WF_PERIODS[i])+'/schedule_trace.log' # Edit here
                                    apprun_file_to_parse= BASEDIR+'/'+WORKLOAD+'/trial_'+str(t+1)+'/c'+str(c+1)+'_f'+str(f)+'-'+sched+'-p'+str(PD_PERIODS[i])+','+str(WF_PERIODS[i])+'/appruntime_trace.log'
                                    print(file_to_parse)

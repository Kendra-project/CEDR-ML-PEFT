...
if __name__ == "__main__":
    ...
    #schedlist = {'EFT':1, 'ETF':2, 'HEFT_RT':3}
    #schedmarkerlist = {'EFT':'^', 'ETF':'s', 'HEFT_RT':'d'}
    #schednamelist = ['EFT', 'ETF', 'HEFT_RT']
    #colorlist = {'EFT': 'green', 'ETF':'red', 'HEFT_RT':'purple'}
    
    #schedlist = {'EFT':1, 'ETF':2, 'MET':3, 'HEFT_RT':4}
    #schedmarkerlist = {'EFT':'^', 'ETF':'s', 'MET':'v', 'HEFT_RT':'d'}
    #schednamelist = ['EFT', 'ETF', 'MET', 'HEFT_RT']
    #colorlist = {'EFT': 'green', 'ETF':'red', 'MET':'orange', 'HEFT_RT':'purple'}

    schedlist = {'EFT':1, 'ETF':2, 'MET':3}
    schedmarkerlist = {'EFT':'^', 'ETF':'s', 'MET':'v'}
    schednamelist = ['EFT', 'ETF', 'MET']
    colorlist = {'EFT': 'green', 'ETF':'red', 'MET':'orange'}
            ...
            for m in range(MMULTS+1):
              for z in range (ZIPS+1):
                for g in range (GPUS+1):
                  if GPUS==0:
                    configlist.append('C'+str(c+1)+'+F'+str(f))
    ...
    metricnames = ['Avg. Cumulative Execution\nTime / App.(ms)', 'Avg. Execution Time / App.(s)', 'Avg. Scheduling Overhead / App.(ms)']
    ...
    for config in configlist:
        ...
        for sched in schedlist.keys():
            ...
            if i == 0:
                p = ax.scatter3D(x,y,z, alpha=1, c = colorlist[sched], s=40, marker=schedmarkerlist[sched], label=schednamelist[schedlist[sched]-1])
            else:
                p = ax.scatter3D(x,y,z, alpha=1, c = colorlist[sched], s=40, marker=schedmarkerlist[sched])
        configcount += 1
        i+=1
    ...
    ax.set_yticklabels(['C2,F0', 'C2,F1', 'C2,F2', 'C2,F3'], fontsize=15)
    ...
    plt.show()
    #plt.savefig("test.png",dpi=1200, papertype='letter')
